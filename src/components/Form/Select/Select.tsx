...
import { ISelect, Options, valueOptions } from './Select.type'
...
const Select = forwardRef((props: ISelect, ref: any) => {
  ...
  const handleSelectOption = (value: Options['value'], event: React.MouseEvent) => {
  ...
  const renderDropdownMenu = () => {
    const getOptions = (listOptions: Options[]) => {
      return listOptions.map((option: Options, index: number) => {
        return (
          <div
            className="options"
  ...
  const getValueInput = (value: Options['value']): string => {
